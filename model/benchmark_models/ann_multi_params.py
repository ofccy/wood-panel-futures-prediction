...
class DayDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Daily frequency features: price sequence of the past 20/look_back days
            ...
            # Convert to float type to ensure the data is numerical
...
class ANNModel(nn.Module):
    def __init__(self, input_dim=look_back + 4, hidden_dims=[256, 128, 64], dropout=0.3):
...
def train_model(model, train_loader, val_loader, epochs=1000, patience=150):
    ...
    for epoch in range(epochs):
        ...
        if rmse < best_val:
            best_val = rmse
            patience_counter = 0
            torch.save(model.state_dict(), 'models/best_ann_multi_params_model.pth')
        # else:
        #     patience_counter += 1
    ...
    if best_val != float('inf'):
        model.load_state_dict(torch.load('models/best_ann_multi_params_model.pth'))
...
def plot_predictions(model, test_loader, test_day_data, scaler):
    ...
    # Convert to a NumPy array and perform inverse normalization
    preds = np.array(all_preds).reshape(-1, 1)
    targets = np.array(all_targets).reshape(-1, 1)
    preds = scaler.inverse_transform(preds)
    targets = scaler.inverse_transform(targets)
    ...
    # Set the date format of the x-axis
    plt.gcf().autofmt_xdate()  # Automatically rotate and format date labels

    # Set the range of the y-axis to ensure it includes all data points
    plt.ylim(min(np.min(targets), np.min(preds)) * 0.95,
             max(np.max(targets), np.max(preds)) * 1.05)

    # Save the chart
    plt.savefig('img/test_predictions_ann_multi_params.png')
    plt.close()
    ...
    prediction_df.to_csv('results/test_ann_multi_params_predictions.csv', index=False)
...
# Ensure that all numerical columns are converted to numerical data types. Perform forced conversion and set errors to NaN
...
# Normalize the `Price` column in the dataset
...
model.load_state_dict(torch.load('models/ann10/best_test_ann_multi_params_model_202502111835_R2_0.8025.pth'))
